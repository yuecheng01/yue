package com.yuecheng.yue.util.anim;
...
public class AvatarImageBehavior extends CoordinatorLayout.Behavior<YUE_CircleImageView> {
    ...
    private void bindDimensions() {
        mAvatarMaxSize = mContext.getResources().getDimension(R.dimen.dp100);
    }


    @Override
    public boolean layoutDependsOn(CoordinatorLayout parent, YUE_CircleImageView child, View dependency) {
        return dependency instanceof Toolbar;  // 依赖Toolbar控件
    }

    @Override
    public boolean onDependentViewChanged(CoordinatorLayout parent, YUE_CircleImageView child, View dependency) {
     ...
     */
    private void shouldInitProperties(YUE_CircleImageView child, View dependency) {
        ...
        if (mFinalHeight == 0)  // Toolbar缩略图高度
            mFinalHeight = mContext.getResources().getDimensionPixelOffset(R.dimen.dp32);
